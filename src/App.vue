  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">网文流派</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">金手指类型</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">网文爆款爽点与黄金三章写作指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 爽文小说金手指与爆点专家');
...
const metricsList = [
  {
    "key": "goldenCheatOriginality",
    "label": "金手指设定新颖度"
  },
  {
    "key": "payoffCatharsisStrength",
    "label": "打脸爽点打斗打爆力"
  },
  {
    "key": "hookExpectationIndex",
    "label": "前三章钩子期待感"
  },
  {
    "key": "pacingRhythmEscalation",
    "label": "冲突升级节奏感"
  },
  {
    "key": "commercialReadershipMatch",
    "label": "网文商业市场匹配分"
  }
];
const guideRules = [
  {
    "title": "金手指机制要直观",
    "advice": "金手指规则在一句话内能解释清楚（如“消费 1 元返还 1000 元”），规则简单才能快速带入爽感",
    "avoid": "切忌设计过于复杂的设定导致读者阅读困难"
  },
  {
    "title": "压得越深弹得越高",
    "advice": "打脸前先让反派充分展示嚣张与偏见，积聚读者期待感，最后用雷霆手段一击打爆",
    "avoid": "避免平铺直叙毫无起伏的无脑打脸"
  },
  {
    "title": "黄金三章留住读者",
    "advice": "前三章必须完成“交代危机-触发金手指-打响第一枪逆袭”，第 3 章结尾留下巨大悬念",
    "avoid": "严禁在前三章大段堆砌设定与历史背景"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[SHUANGWEN_SCORES\][\s\S]*?\[\/SHUANGWEN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[SHUANGWEN_SCORES\](.*?)\[\/SHUANGWEN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('shuangwen_history_records');
...
const saveHistory = () => {
  localStorage.setItem('shuangwen_history_records', JSON.stringify(historyList.value));
};
